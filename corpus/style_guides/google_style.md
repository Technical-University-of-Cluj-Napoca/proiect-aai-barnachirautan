...
...
pylint
warnings are each identified by symbolic name (
empty-docstring
)
...
are to be imported.
y
conflicts with a top-level name defined in the current module.
y
conflicts with a common parameter name that is part of the public
API (e.g.,
features
).
y
...
y
is too generic in the context of your code (e.g.,
from
...
only when
z
is a standard abbreviation (e.g.,
import
numpy as np
).
...
Do not use
assert
statements in place of conditionals or validating
preconditions. They must not be critical to the application logic. A litmus
test would be that the
assert
could be removed without breaking the code.
assert
conditionals are
not guaranteed
to be evaluated. For
...
(
f
'Min. port must be at least 1024, not
{
minimum
}
.'
...
(
f
'Could not connect to service on port
{
minimum
}
or higher.'
...
1024
,
'Minimum port must be at least 1024.'
...
Error
and should not introduce repetition (
foo.FooError
).
Never use catch-all
except:
...
internal by prepending an
_
to the name. If necessary, external access to
...
for an internal use constant or
SIR_LANCELOTS_FAVORITE_COLOR = "blue"
for a public API constant. Constants
must be named using all caps with underscores. See
Naming
below.
2.6 Nested/Local/Inner Classes and Functions
...
closing over a local value other than
self
or
cls
...
resorting to the use of traditional loops,
map()
,
filter()
, or
...
2.7.4 Decision
Comprehensions are allowed, however multiple
for
...
value
in
interesting_iterable
if
a_longer_filter_expression
(
value
)
]
...
operator
module instead of lambda functions. For example, prefer
operator.mul
to
lambda x, y: x * y
.
...
b
is set
to 0. If it is called with two arguments,
b
...
using a property to control attribute access or to calculate a
trivially
...
property descriptor is considered a
power feature
.
...
. Use
if not x:
instead. If you need to distinguish
False
from
None
then chain the
...
(i.e., accidentally handling
None
as 0). You may compare a value which is
known to be an integer (and is not the result of
len()
) against the
integer 0.
Yes
...
context. Prefer the
.size
attribute when testing emptiness of a
np.array
(e.g.
if not users.size
).
...
(a.k.a “the
@
notation”). One common decorator is
@property
, used for
...
decorator runs (at import time, perhaps from
pydoc
or other tools). A
...
operations, there are corner cases where they aren’t atomic (e.g. if
__hash__
or
__eq__
...
data between threads. Otherwise, use the
threading
module and its locking
primitives. Prefer condition variables and
threading.Condition
instead of
...
type hints
. Type-check the code
at build time with a type checking tool like
pytype
.
...
valid code. Use of a
type checker
may reduce your ability to use
Power Features
.
2.21.4 Decision
...
Docstring
summary lines must remain within the 80 character
limit.
...
of a
class
and the first method. No blank line following a
def
line. Use
...
Python uses
docstrings
to document code. A docstring is a string that is the
first statement in a package, module, class or function. These strings can be
extracted automatically through the
__doc__
...
be consistent within a file. The docstring for a
@property
data descriptor
...
starts with “Return”, “Returns”, “Yield”, or “Yields” (e.g.
"""Returns row
...
those are not part of the API). If the function uses
yield
(is a
generator), the
Yields:
...
next()
, instead of the generator object itself that the call evaluates to.
Raises:
...
hanging indent style as described in
Args:
...
the class instance represents. This implies that subclasses of
Exception
...
formatting options. A single join with
+
...
error
(
'Cannot write to home directory, $HOME=%r'
,
homedir
)
No
...
(
f
'Cannot write to home directory, $HOME=
{
homedir
!r}
'
...
warning
(
'Could not remove directory (reason: %r): %r'
,
error
,
workdir
...
1
:
# PROBLEM: also false for float('nan')!
raise
ValueError
(
f
...
warning
(
'Directory already was deleted: %s'
,
workdir
)
try
...
warning
(
'The %s directory could not be deleted.'
,
workdir
)
3.11 Files, Sockets, and similar Stateful Resources
...
A
TODO
comment begins with the word
TODO
in all caps, a following colon, and
...
mind
Deprecated:
application-specific imports that are part of the same
top-level
sub-package as this file. For example:
from
myproject.backend.hgwells
import
time_machine
You may find older Google Python Style code doing this, but it is no longer
required.
New code is encouraged not to bother with this.
Simply treat
...
according to each module’s full package path (the
path
in
from path import
...
...
time_machine
from
myproject.backend.state_machine
import
main_loop
from
otherproject.ai
...
New
unit test
files follow PEP 8 compliant lower_with_under method
names, for example,
test_<method_under_test>_<state>
. For consistency(*)
with legacy modules that follow CapWords function names, underscores may
appear in method names starting with
test
to separate logical components
of the name. One possible pattern is
...
Prefer PEP8-compliant
descriptive_names
for public APIs, which are much
more likely to be encountered out of context.
Use a narrowly-scoped
pylint: disable=invalid-name
directive to silence
...
main()
function, and your code should always check
...
should not be executed when the file is being
pydoc
ed.
3.18 Function length
...
alias
for the type. The last resort is to break after the
...
=
only
for arguments that have both a type annotation and
a default value.
Yes
:
def
...
is an alias for
NoneType
. If an argument can be
None
, it
has to be declared! You can use
|
union type expressions (recommended in new
Python 3.10+ code), or the older
Optional
and
Union
syntaxes.
Use explicit
X | None
instead of implicit. Earlier versions of type checkers
allowed
a: str = None
...
AnyStr
. Use it for
multiple annotations that can be
bytes
or
str
...
or
collections.abc
modules used to support static analysis and type checking,
...
import multiple specific symbols on one line from the
typing
and
collections.abc
modules. For example:
...
type and an existing name in a module, import it using
import x as y
.
from
typing
...
over concrete types like
list
. If you need to use a
concrete type (for example, a
tuple
of typed elements), prefer built-in types
like
tuple
over the parametric type aliases from the
typing
module (e.g.,
typing.Tuple
).
...
. Set an
alias
with a meaningful name, and use the real type name from
this module (any attribute of
Any
is
Any
). Alias definitions should be
...
determine its style. If they use
_idx
suffixes in index variable names, you
